// rank.js — lightweight Elo → rank tier mapping used by both server and client.
// Kept as a tiny shared definition so the UI and API agree on tiers/colors.
...
// Map id -> display value for the client's own tier widget.
export const tierById = (id) => TIERS.find((t) => t.id === id) || TIERS[0];

export default { TIERS, tierForElo, tierById };
